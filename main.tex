...
To prove \Cref{mttt}, we first prove the following lemma.
...
\end{proof}
Now we give the proof of \Cref{mttt} by \Cref{imttt}.
\begin{proof}
...
\end{definition}
\begin{example}
    Let $G$ be a $5$-vertices $2$-graph in \Cref{fig:18-1}. Then $\partial (\mathcal{G})=V(G)$.
\end{example}

\begin{example}
    Let $F$ be a $3$-graph in \Cref{fig:18-1}. Then $\partial (\mathcal{F})$ is a $2$-graph with the blue edges.
\end{example}
...
\end{example}
All these examples have the same idea that we should pack (hyper)edges as compact as possible. And in \Cref{18-e}, the the ordering of the edge set is a colexicographic order which we give the definition as follows. 
...
\begin{exercise}
    ...
    \begin{remark}
        The bound in \Cref{KK} is optimal as the equality holds for initial segments in colex order.
    \end{remark}
\end{exercise}
The idea of the proof of \Cref{KK} is using shifting operation which doesn't change the size of $\mathcal{F}$ and doesn't increased the size of $\partial(\mathcal{F})$. Then induction on the uniformity and the size of the family.
...
\begin{remark}
 ...
 We will leave the following proposition as exercises. 
\begin{proposition}\label{19.5}
 For any   $\mathcal{F}\subseteq \binom{[n]}{k}$  and $2\leq i\leq n$, map $\mathcal{F}\rightarrow S_i(\mathcal{F})$ is injective. In particular, $|\mathcal{F}|=|S_i(\mathcal{F})|$. 
\end{proposition}

 The following proposition states that shifting does not increase the size of shadow. The last equality can be obtained by Proposition \ref{19.5}. 
 \begin{proposition}
 For any   $\mathcal{F}\subseteq \binom{[n]}{k}$  and $2\leq i\leq n$, we have $\partial(S_i(\mathcal{F}))\subseteq S_i(\partial\mathcal{F})$. In particular, $|\partial(S_i(\mathcal{F}))|\leq  |S_i(\partial\mathcal{F})|=|\partial\mathcal{F}|$. 
\end{proposition}

The following proposition will be used in induction. But we first give some notation. Given  $\mathcal{F}\subseteq \binom{[n]}{k}$,  define $\mathcal{F}=\mathcal{F}_1\cup \mathcal{F}_1^c$ where $\mathcal{F}_1=\{F\in \mathcal{F}: 1\in F\}$ and $\mathcal{F}_1^c=\{F\in \mathcal{F}: 1\notin F\}$. Note that $\mathcal{F}_1^c$ is on ground set $[n]\backslash \{1\}$ good for induction, and we remove 1 form all sets in $\mathcal{F}_1$ to form $(k-1)$-uniform.  We use $\mathcal{L}_1=\{F\backslash \{1\}: F\in \mathcal{F}_1\}$ to denote the link element 1 in family $\mathcal{F}$. 

\begin{proposition}\label{19.7}
    If $\mathcal{F}$ is compressed, then \\
    (i) $\partial\mathcal{F}_1^c\subseteq \mathcal{L}_1$, and \\
    (ii) $\partial\mathcal{F}=\mathcal{L}_1\cup \{E\cup \{1\}: E\in \partial\mathcal{L}_1\}$ and so $|\partial\mathcal{F}|=|\mathcal{L}_1|+|\partial\mathcal{L}_1|$.
\end{proposition}
\vskip 0.2em
